...
function getIntensityColor(frequency, maxFrequency) {
    if (frequency === 0)
        return 'bg-gray-50';
...
export function CustomerPainsHeatmap({ data }) {
    const allPains = new Set();
    data.forEach((item) => {
    ...
    const maxFrequency = Math.max(...data.flatMap((item) => item.pains.map((p) => p.frequency)), 1);
    return (<Card>
      <CardHeader>
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr>
                ...
                {painArray.map((pain) => (<th key={pain} className="border p-2 text-center text-xs font-medium">
            ...
            return (<tr key={item.industry}>
                    <td className="border p-2 text-sm font-medium">{translateIndustry(item.industry)}</td>
                    {painArray.map((pain) => {
                    const frequency = painMap.get(pain) || 0;
                    return (<td key={pain} className={cn('border p-2 text-center text-xs', getIntensityColor(frequency, maxFrequency), frequency > 0 && 'text-white font-semibold')}>
                          {frequency}
                        </td>);
                })}
                  </tr>);
