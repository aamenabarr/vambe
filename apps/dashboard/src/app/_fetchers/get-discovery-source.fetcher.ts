import { getDiscoverySourceUseCase } from 'domain-clean/dashboard/use-cases/get-discovery-source.use-case'
...
export async function getDiscoverySource(filters: DashboardFilters) {
  const sellerId =
    filters.seller === FilterValue.ALL ? undefined : await getSellerIdByName(filters.seller)

  return await getDiscoverySourceUseCase({
    sellerId,
