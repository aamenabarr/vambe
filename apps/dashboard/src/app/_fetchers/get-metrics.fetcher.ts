import { getMetricsUseCase } from 'domain-clean/dashboard/use-cases/get-metrics.use-case'
...
export async function getMetrics(filters: DashboardFilters) {
  const sellerId =
    filters.seller === FilterValue.ALL ? undefined : await getSellerIdByName(filters.seller)

  return await getMetricsUseCase({
    sellerId,
